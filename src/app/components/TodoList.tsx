...
const TodoList: React.FC<TodoListProps> = ({ items, onRemove, onToggleComplete }) => {
  return (
    <section className="container mx-auto p-4 max-w-2xl">
      <h3 className="text-xl font-semibold text-gray-700 mb-4">Your Tasks</h3>
      <ul className="space-y-4">
        {items.length === 0 ? (
          <li className="text-center text-gray-500">No tasks yet! Add some tasks to get started.</li>
        ) : (
          items.map((item, index) => (
            <li
              key={index}
              className={`flex items-center justify-between p-4 bg-white shadow-md rounded-lg mb-2 ${
                item.completed ? 'opacity-75 line-through' : ''
              } transition duration-300 ease-in-out transform hover:scale-105`}
            >
              <span className={`flex-1 text-lg font-medium ${item.completed ? 'text-gray-400' : ''}`}>
                {item.text}
              </span>
              <div className="space-x-2 flex">
                ...
                <button
                  className={`px-4 py-2 rounded-lg shadow-md transition-all duration-200 ease-in-out ${
                    item.completed ? 'bg-green-500 text-white hover:bg-green-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
